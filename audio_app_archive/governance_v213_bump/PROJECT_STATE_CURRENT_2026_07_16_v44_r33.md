...
**Date:** July 16, 2026 (PRODUCTION LIVE — SCHEMA v2.13 commercial mapping layer FULLY deployed [additive; item-D/E + Option-B live]; course-card + glossary IMAGE PIPELINE optimized/standardized/live — r33)
**Version:** 4.2 (revision r33)
...
> - **IMAGE ASSETS (2026-07-16 — optimized + standardized + LIVE):** Course Select **25 course cards** → WebP (941×1672), `tier_key.webp` names, live in bucket `course-cards`; **no DB card-image column** → frontend derives file from card_id (`${card_id.replace(':','_')}.webp`), wiring deferred to frontend. Glossary **139 term images** → WebP (≤1024px), `<term-slug>.webp`; **`glossary_media.url` REWRITTEN to `.webp` on prod** (backup `glossary_media_backup_20260716`, 139 rows); 136 live, **3 XLR (XLRM/XLRF/XLR Cable) pre-wired pending art**; 0 unreferenced. Assets ~235MB→~9MB. Detail → TRACKER r32; maps → `COURSE_CARD_MAP_2026_07_16.md` + `ape_glossary_media_rename_2026_07_16.json`.
