...
**Date:** July 11, 2026 (PRODUCTION LIVE — SCHEMA v2.13 commercial mapping layer deployed [additive; item-D held] — r31)
**Version:** 4.2 (revision r31)
**Author:** Prof. Channing Booth, San Diego Miramar College

> **🟢 CURRENT STATE (2026-07-11 — SCHEMA v2.13 COMMERCIAL MAPPING LAYER DEPLOYED to prod `yjgolswjggmlpeowvtxr` [additive; item-D HELD]; supersedes the 2026-07-10 block below).**
> Path B commercial-first backend landed on production as **4 tracked migrations, additive-only** (no academic table altered). Verified on prod; `get_advisors` clean (no new ERROR).
...
> - **Migrations:** `v213_core_ddl_mapping_layer` · `v213_anon_catalog_grants_and_glossary_view` · `v213_seed_public_courses_and_topics` · `v213_advisor_cleanup_defer_glossary_view`. Rollback staged.
> - **HELD:** item-D `common_mistakes` gate + `glossary_full_v` view — ships WITH the client release (client reads common_mistakes from the view; else institutional 403).
> - **CANDIDATE (blocked):** `start_quiz_attempt` v3 (optional `p_public_course_id` param ruled) — blocked on design Qs (commercial has no enrollment/progress rows; clamp over `public_course_topics.seq`). Live v2 untouched.
