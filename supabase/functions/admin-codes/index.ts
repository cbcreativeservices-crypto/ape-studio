...
// CORS: the console page runs as a local file (origin "null") or any origin,
// so the API allows any origin. Origin is NOT a security boundary here — every
// action still requires a valid session, AAL2, the admin allowlist, and the
// passphrase (no cookies/credentials are used, so `*` is safe).
const CORS = {
...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    // A Headers instance (not a plain object) so the content-type reliably
    // sticks — the object form was arriving at the browser as text/plain and
    // the page rendered as source (2026-09-07).
...
<style>
  ...
  *{box-sizing:border-box} html,body{margin:0}
  ...
  .grp{font-family:var(--mono);font-size:11px;letter-spacing:.2em;text-transform:uppercase;color:var(--amber);margin:18px 0 4px}
  ...
  <div class="hidden" id="console">
    ...
    <div class="card">
      ...
      <div class="toolbar">
        ...
        <button class="ghost mini on" id="viewList">List</button>
        <button class="ghost mini" id="viewCat">By plan</button>
      </div>
...
let loadedRows = [];
let viewMode = "list";
const planLabel = (d) => d===null?"lifetime":(d===365?"year":(d===30?"month":d+"d"));
function codeRow(r){
  return '<tr><td class="code">' + r.code + '</td><td>' + planLabel(r.grant_days) + '</td><td>' +
    ...
    '<button class="ghost mini" data-code="' + r.code + '" data-act="' + (r.active?"deactivate":"reactivate") + '">' + (r.active?"Disable":"Enable") + '</button></td></tr>';
}
function codeTable(rows){
  return '<table><thead><tr><th>Code</th><th>Plan</th><th>Used</th><th>Note</th><th>Status</th><th></th></tr></thead><tbody>' +
    rows.map(codeRow).join("") + '</tbody></table>';
}
function renderCodes(){
  const wrap = $("listWrap");
  $("viewList").classList.toggle("on", viewMode === "list");
  $("viewCat").classList.toggle("on", viewMode === "cat");
  if (!loadedRows.length){ wrap.innerHTML = '<p class="hint">No codes yet.</p>'; return; }
  if (viewMode === "cat"){
    const groups = [["lifetime","LIFETIME"],["year","1 YEAR"],["month","1 MONTH"]];
    let html = "";
    for (const [key,label] of groups){
      const g = loadedRows.filter((r) => planLabel(r.grant_days) === key);
      if (g.length) html += '<h3 class="grp">' + label + ' · ' + g.length + '</h3>' + codeTable(g);
    }
    const others = loadedRows.filter((r) => !["lifetime","year","month"].includes(planLabel(r.grant_days)));
    if (others.length) html += '<h3 class="grp">OTHER · ' + others.length + '</h3>' + codeTable(others);
    wrap.innerHTML = html;
  } else {
    wrap.innerHTML = codeTable(loadedRows);
  }
  wrap.querySelectorAll("button[data-code]").forEach((b) => b.onclick = async () => {
    b.disabled = true;
    try { await call(b.dataset.act, { code: b.dataset.code }); await $("refreshBtn").onclick(); }
    catch(e){ alert(e.message); b.disabled = false; }
  });
}
$("viewList").onclick = () => { viewMode = "list"; renderCodes(); };
$("viewCat").onclick = () => { viewMode = "cat"; renderCodes(); };
$("refreshBtn").onclick = async () => {
  if (!$("pass").value){ $("listWrap").innerHTML = '<p class="hint">Enter your passphrase first.</p>'; return; }
  $("refreshBtn").disabled = true;
  try{
    const body = await call("list");
    loadedRows = body.rows || [];
    renderCodes();
