...
| Any data linked to a user identity? | **No — 100% not linked.** Neither SDK receives any user identifier. Sentry never calls `setUser` (the telemetry module has no auth access at all, pinned by `test/telemetry.test.ts`) and `beforeSend` removes any `user` object as a backstop. Aptabase has no user field. Owner ruling 2026-09-16 ("Option B": fully anonymous crash reporting). |
...
- **Apple App Privacy** → *Data Not Linked to You*: Crash Data, Performance Data (release-health), Product Interaction, Other Diagnostic Data. *Data Linked to You*: **none.** **Tracking: No.**
- **Google Data Safety** → *Collected*: Crash logs, Diagnostics (app performance), App interactions. *Shared*: none. *Encrypted in transit*: yes (HTTPS). *Deletion*: nothing collected carries a user identifier, so there is no per-user data to delete.
