...
| 3 | Bluehost → `.online` → Nameservers | **Canary.** Switch `.online` only. Two records, redirect-only, no mail — a failure here costs nothing. Expect resolvers to take up to ~48h to all move across; that is the registry delegation TTL, not a fault. |
