...
| 6 | Bluehost → `.com` → Nameservers | ⏳ **NEXT, and the only one that can hurt.** Switch **last** — this one carries Google Workspace mail, both DKIM keys and the live site. Two gates before it: (a) confirm `.co` has propagated the way `.online` did, on two resolvers; (b) run the step 3a pre-flight against the real payload, not just the apex — `nslookup -type=MX proaudiotrainingacademy.com ns1.vercel-dns.com` and the same for `google._domainkey` / `resend._domainkey` TXT — so the mail records are proven to be served from the destination zone *before* the delegation moves, not after. Unlike the two redirect domains, a mistake here bounces real mail. |
