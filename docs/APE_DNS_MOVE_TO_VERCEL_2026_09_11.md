...
| 7 | Terminal + mail | ⏳ **DNS SIDE COMPLETE 2026-09-11 — awaiting the one check a machine cannot make.** ✅ **All three domains have propagated: `.com` delegation now reads `ns1/ns2.vercel-dns.com` on BOTH Google `8.8.8.8` and Cloudflare `1.1.1.1`.** Verified through the public resolvers, not just at the nameserver: both DKIM keys **byte-identical to the destination zone** (`google._domainkey` 408 / `resend._domainkey` 218, MATCH on both resolvers), `MX → 1 smtp.google.com`, `send`/`rsend` → `*.forge.rmta.net`, `_dmarc` → `v=DMARC1;p=none`, apex `308 → www`. ⚠️ **Two harmless leftovers on Google only, and they are a DIFFERENT CLOCK from the delegation:** it still served the stale `MX 10 proaudiotrainingacademy.com` and the old apex `216.198.79.1`, because those are *record* data cached under the old Bluehost zone's own TTLs (1h/4h) — the NS delegation TTL moved first. Cloudflare had already dropped both, which is what fully settled looks like. Neither can misroute mail: the junk MX points at an apex that accepts no mail, and delivery goes to `pref 1` Google regardless. **STILL OPEN — the owner must send a real message to `info@proaudiotrainingacademy.com` from a NON-Workspace address and confirm it ARRIVES.** Every DNS check above is green and none of them proves delivery. |
