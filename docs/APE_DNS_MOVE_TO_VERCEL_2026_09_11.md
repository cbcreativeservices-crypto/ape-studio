...
| 11 | Resend dashboard | ✅ **CHECKED 2026-09-11 — Resend is happy, and NO re-verify action was taken, deliberately.** Domain status **Verified**, all **7 record statuses Verified**, nothing pending/failed/partial, *Enable Sending* ON. Resend's own Records tab states exactly three requirements and all three are live from the Vercel zone: `TXT resend._domainkey`, `CNAME send → send.forge.rmta.net`, `CNAME rsend → rsend.forge.rmta.net`. **The decisive check: the DKIM key Resend's dashboard says it EXPECTS is byte-identical to what the Vercel zone SERVES** (218 chars, SHA-256 `bfc9b5ba5aa11ca2` on both sides) — so Resend's "Verified" is not a stale badge, it matches present reality. ⚠️ **Two things are cosmetically stale and neither matters:** the dashboard still reads *PROVIDER: Bluehost*, and the event timeline still reads *DNS verified Aug 30, 8:45 AM* — both predate the move. |
| 11a | — | ⚠️ **Why "Restart" was NOT clicked, so nobody clicks it casually later.** The ONLY re-verification control Resend offers sits in the domain's `…` menu **directly above "Delete domain"**, and it is undocumented on the points that matter: Resend's docs describe *restart verification* as the remedy for a verification that **has not completed after 72 hours**, and say nothing about whether it preserves or **regenerates the DKIM keypair**. On a live sending domain that is already fully verified, pressing it risks invalidating the `resend._domainkey` value now published in the Vercel zone — silently breaking DKIM on the weekly-concept email — in exchange for refreshing a cosmetic provider label. Asymmetric, so it was left alone. **If a future change genuinely needs it, update the `resend._domainkey` TXT in Vercel IMMEDIATELY afterwards and re-run the byte-comparison above.** |
