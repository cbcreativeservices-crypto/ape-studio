...
7. ◑ PARTIAL — CODE FIXES DONE, OWNER ITEMS OUTSTANDING:
   ...
   - ⏳ OWNER: `subjectMeta` is PLACEHOLDER + v2-keyed copy (render already null-guards unmatched subjects, so no blank-row bug) — ratify real v3-keyed copy, or ask me to hide behind a flag. Re-ratify the (now-correct) compressor & RF copy.
