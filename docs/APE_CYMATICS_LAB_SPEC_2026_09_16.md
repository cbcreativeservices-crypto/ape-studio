...
### Rack pass (2026-09-17, judge panel re-run — APE_LAB_UX_PROPOSAL_2026_08_23 §3, JUDGE_PANEL.md method)
Three adversarial judges (pedagogy / engineering-on-this-codebase / design language) re-read every screen against the Rack Unit law *reading may scroll; operating may not*. Verdicts as built:
- **On the rack** (`modules/rackLayout.tsx` → `RackUnit`, host `CymaticsModuleScreen` `RACK_MODULES`): **Nodes** (FREQ lane + sticky JUMP TO A MODE chooser, SHAPE / VIEW trays, PLAY / SILENT toggles; bezel MODE · DRIVE · LINES · RES, RES tap lands the nearest mode), **Harmony in Motion** (BASE lane 55–440 Hz, DETUNE, RATIO / VIEW trays; bezel A · B · INTERVAL · BEAT), **Change One Thing** (VARY / A / B / VIEW trays, two plates on one glass, PREDICT card in the well; bezel LOCKED · A · B), **Other Cymatic Systems** (SYSTEM tray, HARMONIC lane for string / pipe; bezel SYSTEM · MODE · NODES · FAMILY). Stage pinned, first-move caption outside a collapsible LAB NOTES, guided-lesson row at the bottom of the notes.
- **Kept as documents:** Intro, Harmonics vs Plate Modes, Evidence vs Myth, Guided Experiments (steps now fold behind a `▸ N STEPS` preview row).
- **Studios:** dock values on the ≤5-character budget (`Al240`, `S+H`, `Ø150`, `14″M10`…), well action chips moved into the DRIVE / STRIKE / SAND trays, TAP TO LAND retired in favour of the RES / RESPONSE bezel cell's tap, Liquid SHAKE on the bezel (λ to the well), MIN_FONT 12 everywhere. `RackUnit` reserves 350 px under the stage so a 4-cell bezel + caption fit above the dock.
- **Not applied (owner judgment):** the lab home as a rack of three units; Membrane conditional DRUM / CONE dock + TENSION fader; ExperimentWell above LAB NOTES (a LabShell change); per-param `hideDragTag` in the kit; `markLabUnit` for cymatics; Low-Light auto-motion gating; illustration passes (drum shell, lamp, cutaway).

### Guided experiments run IN the studio (2026-09-17, owner report)
The experiments module is a MENU; the activity happens in the instrument. `LabShell` exposes `rack.wellTop` — pinned at the top of the well, above the first-move caption and outside every disclosure — and each studio renders `ExperimentWell` there, so the prediction and the tick-off steps are the first thing under the stage on arrival. The card carries the series: `EXPERIMENT n / 17` with PREV / ALL 17 / NEXT, which replace the studio route in place and cross studios when the next experiment lives in another one (`NEXT · DISH`); back exits to module 8 from anywhere in the run. `experimentRoute()` in `presets.ts` is the one studio+preset map. Module 8 leads with START THE SERIES and keeps the seventeen cards as a jump-in-anywhere menu.
...
- **Phase 4 as built (2026-09-17, Fable design + build; device pass pending)** — design of record `docs/APE_CYMATICS_PHASE4_DESIGN_2026_09_17.md`. ONE route `CymaticsGallery` (`labs/cymatics/gallery`), fourth button on the lab home, four modes inside it: BROWSE (filters, ★, select 2–4 to compare) · OPEN (line-art figure on the object face, name + notes, the settings block, OPEN IN STUDIO → the studio route with `{ saved: id }` restores the exact configuration, COLOUR, DUPLICATE, DELETE, EXPORT) · ART (the art board on the Rack Unit: FILL swatches + custom + erase · STYLE solid / gradient + line colour · WEIGHT lane · PAPER · SYM 1–8; UNDO/REDO on the bezel; autosave) · COMPARE (2 side by side or 4 in a 2×2 on one canvas, captions, the comparison as data, export). Core: `src/features/cymatics/{contours,patternStore,patternField,figure,svgExport}.ts` — regions are the sign components of the node field, a region's boundary comes from padded marching squares on the signed magnitude (lands on the true nodal line), symmetry fill = rotated-tap lookup, `ART_N = 96`. Renderer = `react-native-svg` (`PatternFigure.tsx`, capture-safe on Android, the same path data the SVG export writes). Exports (`ExportPanel.tsx` + `galleryExport.ts`): SHARE PNG / SAVE to Photos (Harmonograph gates), PRINT + PDF Letter / A4 / Square (expo-print HTML, page picker built now), SVG (text share today, file share with sharing), ART PRINT hides data / LAB PRINT prints the settings block with "Simulation". Studios: SAVE in the header (`HeaderTextButton`), "Saved to the Gallery ✓ · Open the gallery ›". Storage `ape:cymatics:patterns:v1` + `ape:cymatics:artwork:v1` (separate, quarantine on damage). 10 tests in `test/cymaticsGallery.test.ts`. `PLANNED_AREAS` is now empty.
...
| 3b | Rack Unit learning-design pass over all eight modules + three studios — **DONE 2026-09-17** (`79237167`; see §3 *Rack pass*) | Pixel: Nodes / Harmony / Change / Systems render stage-pinned with live bezels and the dock at the bottom; dock keys ≤5 chars |
| 4 | Gallery + Art Studio + exports — **BUILT 2026-09-17** (tsc clean, 1222 tests; DEVICE PASS PENDING — the owner stopped the Fable session before testing; the script is §10 of `docs/APE_CYMATICS_PHASE4_DESIGN_2026_09_17.md`) | export parity with Harmonograph; SVG opens in a vector editor — Pixel: save from each studio, reopen exactly, colour with undo/redo, exports disabled with the honest note, SVG share works, compare 2 and 4, zero device errors |
