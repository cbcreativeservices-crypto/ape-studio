...
export const FEEDS: Record<FeedId, FeedSpec> = {
  ...
  FILL: { id: 'FILL', name: 'Front fill', bus: 'Matrix (from main)', band: 'full' },
  DELAY: { id: 'DELAY', name: 'Delay', bus: 'Matrix (from main, delayed)', band: 'full' },
...
export type PlacedSpeaker = {
  ...
  /** Aim in degrees, 0 = straight into the audience, negative = toward stage-left. */
  aimDeg?: number;
...
export const OUTPUT_CONFIGS: readonly OutputConfig[] = [
  ...
  },
  {
    ...
    changes: 'Coverage widens; where the two wedges overlap the same signal arrives twice from different distances — the comb-filter zone down the middle is the price.',
  },
  {
    id: 'stereo',
    name: 'Stereo',
    ...
    changes: 'A stereo image for the centre seats; the sides hear mostly one channel. Panned sources drop level for the listener on the far side.',
  },
  {
    id: 'lcr',
  ...
  {
    id: 'two-one',
    name: '2.1 — left, right and subwoofer',
    layout: [TOP_L, TOP_R, { slot: 'subC', feed: 'SUB', kind: 'sub' }],
    what: 'A stereo pair plus one subwoofer feed. The console (or the processor) decides what the sub receives.',
    when: 'Music systems from a bar up. One sub position avoids two subs cancelling each other across the room.',
    changes: 'The tops are relieved of low frequencies and play cleaner and louder; the low end comes from one place and covers evenly.',
  },
  {
    id: 'mono-sub-out',
    name: 'Mono mains with a separate subwoofer output',
    layout: [TOP_L_MONO, TOP_R_MONO, { slot: 'subC', feed: 'SUB', kind: 'sub' }],
    what: 'A mono main feed to the tops and a SEPARATE sub output — not the tops’ pass-through.',
  ...
  },
  {
    ...
    changes: 'Two sub positions, one signal: the low end is even and predictable; a centre-front listener gets both subs in step.',
  },
  {
    id: 'stereo-subs',
  ...
  },
  {
    ...
    changes: 'The front rows get a direct, intelligible source at a modest level; the fills are matrix-fed so they can be delayed and equalised on their own.',
  },
  {
    id: 'delays',
...
export const SYSTEM_TYPES: readonly SystemType[] = [
  {
    ...
    layout: [TOP_L_MONO, TOP_R_MONO],
    chain: ['Microphones and DIs', 'Small console', 'Two powered loudspeakers on stands'],
  ...
  },
  {
    ...
    layout: [TOP_L, TOP_R, { slot: 'subL', feed: 'SUB', kind: 'sub' }, { slot: 'subR', feed: 'SUB', kind: 'sub' }],
    chain: ['Sources', 'Console', 'Loudspeaker processor', 'Amplifier rack', 'Passive tops and subs'],
  ...
  },
  {
    ...
    layout: [{ slot: 'mainL', feed: 'M', kind: 'top', aimDeg: 14, coverDeg: 120 }, { slot: 'mainR', feed: 'M', kind: 'top', aimDeg: -14, coverDeg: 120 }],
    chain: ['Sources', 'Small mixer (often in the base)', 'Column tops on sub bases'],
  ...
  },
  {
    ...
    chain: ['Stagebox', 'Digital console', 'Processor', 'Amps', 'Tops, subs, front fills', 'Wedges'],
  ...
  },
  {
    ...
    chain: ['Many inputs, many operators', 'Digital console', 'Processor with zones', 'LCR mains, delays, under-balcony fills'],
  ...
  },
  {
    ...
    chain: ['Wireless microphones and playback', 'Console with automatic mixing', 'Processor', 'Distributed or delayed loudspeakers', 'Recording and stream feeds'],
    when: 'Speech from many wireless microphones, video playback, a stream — every word clear at a modest level.',
    notWhen: 'A band — the design has no low end and no monitor system to speak of.',
  ...
  },
  {
    ...
    chain: ['Stagebox', 'Console', 'Processor', 'Amps', 'Mains, subs, delay towers'],
  ...
  },
  {
    ...
    chain: ['Stagebox and splits', 'FOH and monitor consoles', 'Networked amplifiers with processing', 'Flown arrays, sub arrays, fills, delays'],
    ...
    notWhen: 'Anything that fits in a van. A line array below a certain length is just several boxes stacked.',
    scale: '2,000 upward',
  },
  {
    id: 'distributed',
    ...
    when: 'Low ceilings, long rooms, background music and paging — restaurants, terminals, lobbies. Every listener is near a loudspeaker.',
  ...
  },
  {
    ...
    layout: [TOP_L, TOP_R, { slot: 'frontFillL', feed: 'FILL', kind: 'fill', aimDeg: 0, coverDeg: 90 }, { slot: 'frontFillR', feed: 'FILL', kind: 'fill', aimDeg: 0, coverDeg: 90 }, { slot: 'delayL', feed: 'DELAY', kind: 'delay', aimDeg: 0, coverDeg: 90, delayMs: 60 }, { slot: 'delayR', feed: 'DELAY', kind: 'delay', aimDeg: 0, coverDeg: 90, delayMs: 60 }],
    chain: ['The main system', 'Matrix outputs', 'Processor delay and EQ per fill', 'Small cabinets where the mains cannot reach'],
...
export const VENUE_CASES: readonly VenueCase[] = [
  ...
  { id: 'v-wedding', venue: 'A wedding in a marquee: speeches, a DJ, a hard floor and a fabric roof.', correct: 'column', why: 'A column array’s wide horizontal and narrow vertical coverage puts sound on the guests and not on the roof; the sub base carries the DJ.' },
