...
export const LAB_LESSONS: Record<LabId, LabLesson> = {
  ...
  compression: {
    ...
    controls: [
      ...
      { key: 'ratio', name: 'Ratio', definition: 'How hard it squeezes above the threshold: 2:1 is gentle glue, 8:1+ acts like a limiter. For every N dB the input rises ABOVE the threshold, the output rises only 1 dB — below the threshold nothing changes.', range: '2:1 … 20:1' },
  ...
  mic: {
    ...
    controls: [
      ...
      { key: 'polar_pattern', name: 'Polar pattern viewer', definition: 'The pattern is the mic’s sensitivity map by direction — r(θ) = |A + B·cosθ| drawn exactly. Drag the speaker cabinet around the mic and read its pickup; switch OMNI · CARDIOID · SUPER · HYPER · FIGURE-8 to move the nulls.' },
  ...
  speaker: {
    ...
    controls: [
      ...
      { key: 'dispersion', name: 'Dispersion (H° × V°)', definition: 'The nominal wedge a cabinet controls — 60°×40° throws far and narrow, 120°×60° covers wide and close. Narrower patterns need more careful aim; wider ones spread the same power over more area, so they are quieter at EVERY distance — not faster-falling with distance, which is the same inverse-square law for both.' },
  ...
  meter: {
    ...
    controls: [
      ...
      { key: 'waterfall_axes', name: 'Waterfall axes (CSD)', definition: 'Frequency across, level up, TIME stepping TOWARD you: the loud start (t = 0) stands tall at the back, and each slice in front of it is the spectrum a moment later. The front edge is the latest instant — the range decaying toward you IS the decay.' },
  ...
  controls: [
    { key: 'display', name: 'What the display shows', definition: 'The plate from above. PARTICLES: sand on the plate. HEAT MAP: how far each point moves, on the Academy amplitude ramp (black = still → blue = a little → red = the most); off a resonance the whole map goes dark, because the plate barely moves. PHASE: amber regions rise while violet regions fall — and neither is a ramp colour, because this view shows DIRECTION, not level (a violet lobe is moving as hard as an amber one). NODE LINES: the figure of the nearest mode — what WOULD form; it fades between resonances. 3D PLATE and CROSS-SECTION: the motion, strobed to a few hertz — the real plate moves at the drive frequency. Every view is derived from the same simulated field; the badge names the model: RITZ = the standard free-plate approximation (no exact solution exists); BESSEL = the exact circular functions with tabulated eigenvalues (the frequencies a disc’s modes sit at); FEM = finite elements, a numerical solution of the plate equation on the actual outline.' },
    ...
    { key: 'shape', name: 'Shape', definition: 'Square, rectangle or disc — plus triangle, hexagon, ring (free or clamped hub), bell plate, guitar and violin tops. Shape sets the FAMILY of figures: crossed and diagonal lines on a square, nodal circles and diameters on a disc, three- and six-fold figures on the triangle and hexagon, and the bending, torsion and ring modes luthiers tune by tap tone on the instrument plates. The first three are analytic; the rest are solved numerically (finite elements) and looked up, with their edge condition fixed by the solution.' },
    ...
    // ── Liquid Studio (Phase 2) ──
    { key: 'liquid_display', name: 'What the liquid display shows', definition: 'THE RIG: the apparatus from the side — lamp, dish, liquid, platform, shaker; the platform bobs at the drive frequency while the surface answers at half of it. LIQUID SURFACE: the lit, glossy surface from above. HEIGHT MAP: displacement in the Academy ramp. CONTOURS: iso-height lines over the dimmed height map. REFRACTION: the classic light-through-water caustic web. 3D and CROSS-SECTION: the motion, strobed to a few hertz. Wherever height is drawn as colour it is the Academy amplitude ramp — dark = still, blue = small, red = the largest motion — the same ramp as every meter and heat map in the app. All derived from one simulated field; the badge names the approximation.' },
    { key: 'faraday', name: 'Faraday waves (the half-frequency response)', definition: 'Shake a liquid up and down and, above a threshold, its surface breaks into standing waves that oscillate at HALF the drive frequency — a parametric resonance first reported by Michael Faraday in 1831. The RESP readout shows that half-frequency; λ is the wavelength the dispersion relation gives at it.' },
    { key: 'threshold', name: 'Onset threshold (a/a꜀)', definition: 'The vertical acceleration needed before the subharmonic wave can grow against damping. Below it (a/a꜀ < 1) the surface only rides the platform; above it patterns appear. Viscosity, a thin layer (bottom friction) and a pinned rim all raise it. Values are the low-viscosity Faraday estimate — Approximated, not measured.' },
    { key: 'acceleration', name: 'Shake (vertical acceleration)', definition: 'How hard the platform drives the dish, in g. The readout also shows the platform’s travel: at a fixed acceleration, higher frequencies need far less displacement (a = A·(2πf)²). This is the control that walks the surface up the ten-stage ladder.' },
    { key: 'liquid', name: 'Liquid', definition: 'Eight presets with their real density, viscosity and surface tension. Viscosity raises the threshold and favours stripes; surface tension shortens the wavelength; density enters the capillary term. Thick oils and gels are honestly “no pattern in this rig’s range”; cornstarch shear-thickens — it stiffens the harder it is shaken.' },
    { key: 'viscosity', name: 'Consistency ↔ viscosity', definition: 'Plain words: how thick the liquid feels. Physics: dynamic viscosity μ (Pa·s), and ν = μ/ρ. It sets the bulk damping 2νk², so it is the main reason the onset threshold differs between liquids. Warmer = thinner (about −2.5 %/°C for water-like liquids).' },
    { key: 'depth', name: 'Liquid amount ↔ depth', definition: 'A thin layer is damped by the bottom boundary layer (higher threshold) and its waves are shortened by tanh(kd). Deeper liquid approaches deep-water dispersion. Change the depth at a fixed frequency and both λ and the threshold move.' },
    { key: 'dish', name: 'Dish (container)', definition: 'Shape, size, bottom and rim. When the wavelength is comparable to the dish, the dish’s OWN standing modes (rings, lobes, spokes, or a checkerboard in a square) set the pattern; in a big dish the bulk Faraday lattice (squares, stripes, hexagons) wins. A shallow bowl bottom shortens the wave toward the edge.' },
    { key: 'contact_line', name: 'Contact line (rim)', definition: 'Whether the liquid wets and pins to the rim (most dishes) or meets it freely. A pinned rim adds meniscus damping — in small dishes it can be the largest damping term — and selects different mode shapes (J_n zeros instead of J_n′ zeros).' },
    { key: 'stages', name: 'Behaviour stages (1–10)', definition: 'Flat → gentle sloshing (a dish mode near the drive frequency) → small edge ripples → pattern onset → stable standing pattern → mode transition → competing patterns → unstable, travelling fronts → chaotic surface → splashing. The bands are illustrative divisions of a/a꜀; the physics inside each is the real Faraday picture. The wall height sets where splashing starts.' },
    { key: 'waveform', name: 'Excitation waveform', definition: 'Sine isolates one drive frequency and is the primary demonstration. Square and triangle add odd harmonics (played by the additive engine on engine ≥ 3); pulse drives tone bursts. Onset still keys on the fundamental.' },
    // ── Membrane & Loudspeaker Studio + Phase 3 modules ──
    { key: 'membrane_display', name: 'What the drum display shows', definition: 'THE DRUM: the head from above, lit, bulging and dipping in the driven mode (strobed). HEAT MAP: how far each point moves, on the Academy amplitude ramp — dark between resonances. PHASE: amber rises while violet falls — neither is a ramp colour, because this shows DIRECTION, not level. NODE LINES: the nodal diameters and circles of the nearest mode. 3D HEAD and CROSS-SECTION: the motion, strobed. LOUDSPEAKER: a cutaway driver plus the cone seen from the front, its motion drawn from the same membrane engine with a stiffness profile (Illustrative stages). The badge names the model: clamped-membrane Bessel modes are Calculated; the kettle loading is Approximated.' },
    { key: 'head', name: 'Head material', definition: 'What the drumhead is made of sets its areal density σ (kg/m²) and its damping Q. Frequency goes as 1/√σ, so a lighter film pitches every mode higher at the same tension; calfskin and latex are heavier or lossier and ring shorter; Kevlar is tensioned very high and rings hard.' },
    { key: 'tension', name: 'Tension and size', definition: 'A membrane has no stiffness of its own — only tension T (N/m). Every mode’s frequency is (j / 2πR)·√(T/σ): four times the tension doubles the pitch; twice the diameter halves it. The FUNDAMENTAL readout on the bezel is the (0,1) mode; tuning a drum is setting T with the lugs.' },
    { key: 'strike', name: 'Strike point', definition: 'A mode is excited in proportion to how much it moves under the mallet. A dead-centre strike sits on every nodal diameter and wakes only the ring modes (the thud); the timpanist strikes about a quarter of the way in to drive the (1,1) mode that carries the pitch. Drag the mallet across a nodal line and watch the mode drop out.' },
    { key: 'kettle', name: 'Timpani kettle', definition: 'A bare membrane’s modes sit at the Bessel ratios 1 : 1.59 : 2.14 : 2.30 : 2.65 — inharmonic, so no clear pitch. Put a closed bowl under it and the air loading pulls the (1,1), (2,1), (3,1), (4,1) modes to about 1 : 1.5 : 2 : 2.5 (Rossing’s measurements): a harmonic series missing its fundamental, which the ear supplies. That is why a timpani plays a note and a snare does not. Approximated — the measured ratios are applied to the principal modes, a plain air-mass factor to the rest.' },
    { key: 'cone', name: 'The loudspeaker cone', definition: 'A cone is meant to move as one rigid piston. Below its suspension resonance f_s it is stiffness-controlled and barely moves; at f_s it travels furthest; above it, in the mass-controlled piston band, excursion falls as 1/f² while the radiated pressure stays flat — the reason a driver has a flat passband. ka = πfD/c is the beaming number: above ka ≈ 1 the output narrows. The six stages are Illustrative; f_s, ka and the 1/f² law are Calculated.' },
    { key: 'breakup', name: 'Cone breakup', definition: 'Above the cone’s first bending frequency it no longer moves as one: the rim decouples from the coil (edge flexing), then bell-like radial modes appear (the same J_n cos nθ family as the drumhead), then several ring at once — peaks, dips and distortion. Paper 8" cones break up near 2 kHz; a stiff metal dome holds to 20 kHz and then breaks violently. Crossovers hand over below breakup.' },
    { key: 'ratio', name: 'Frequency ratios', definition: 'Musical intervals are ratios between two frequencies, not properties of either one: 2 : 1 an octave, 3 : 2 a fifth, 4 : 3 a fourth, 5 : 4 a major third. Two tones at a small-integer ratio line up again and again — their sum repeats, their Lissajous figure closes. Played exactly as harmonics of a shared base through the additive engine.' },
    { key: 'beats', name: 'Beats', definition: 'Two tones a little apart slide in and out of step |f₂ − f₁| times a second: loud where they agree, quiet where they cancel. A beat is a slow swell in loudness, not a third tone — the spectrum shows two lines and nothing between. Zero beats is how a tuner knows two strings are in tune. Shown only on builds whose engine plays one locked pair at a time.' },
    { key: 'lissajous', name: 'Lissajous figure', definition: 'Plot one tone on x and the other on y. A locked ratio n₁ : n₂ closes into a figure with n₂ lobes across and n₁ down; detune the pair and the figure precesses at the beat rate. The Harmonograph draws the same geometry with pendulums.' },
    { key: 'systems', name: 'Other cymatic systems', definition: 'Every system that shows its vibration shows the NODES of one of its own modes: a string’s still points, a pipe’s pressure nodes (cork dust, a flame), a liquid’s Faraday pattern, grains on a cone above breakup, a bell’s nodal meridians, beads hanging at the nodes of a standing wave in air. Strings and air columns are harmonic and so have a pitch; membranes, plates, bells and dishes are not, unless a kettle or a bell-founder pulls their modes toward integers.' },
    { key: 'change_one', name: 'Change one thing', definition: 'The lab’s test of its own claim: two plates, one tone locked to a resonance of plate A, and exactly one difference between A and B. If B goes dark at the same frequency, the tone did not change — the plate did. Material, size, thickness, shape, driver position and support all do this; damping is the one that blurs the resonance instead of moving it.' },
    { key: 'dual_liquid', name: 'Second frequency', definition: 'Two-frequency forcing is how the quasiperiodic “superlattice” patterns were discovered (Edwards & Fauve). Heard as a true two-tone sum on engine 8; shown only on earlier builds. RELATIVE PHASE is a visual control here.' },
    // ── Pattern Gallery & Art Studio (Phase 4) ──
    { key: 'gallery', name: 'Pattern Gallery', definition: 'Every SAVE from a studio lands here with its exact experiment state — the plate or dish or head, the drive, the level, the view — and the honesty badge it carried. Reopen it and the studio comes back to that configuration. The gallery itself draws the figure as line art on the object; the live views live in the studio.' },
    { key: 'save_pattern', name: 'Saving a pattern', definition: 'SAVE stores numbers, not a picture: the studio’s full state, so the figure is reproducible. Artwork you add later is stored separately and never changes the science underneath.' },
    { key: 'art_fill', name: 'Fill', definition: 'Tap a region — one of the areas enclosed by nodal lines — to fill it. Regions are the parts of the plate moving together, in phase; a fill never crosses a still line because the still line is the boundary. ERASE clears a region; UNDO and REDO live on the bezel.' },
    { key: 'art_style', name: 'Solid or gradient', definition: 'Solid paints the region flat. Gradient shades it from its centre outward, which reads as height: the antinode in the middle, the still line at the edge. NODAL LINES here sets the line colour.' },
    { key: 'art_weight', name: 'Line weight', definition: 'The nodal lines’ stroke, 0–4 px. At zero they vanish and only the fills speak — the figure is still there, drawn by the colours’ edges.' },
    { key: 'art_paper', name: 'Paper', definition: 'What sits under the figure: the object’s own face (aluminum, brass, the drumhead, the liquid), a plain colour, or nothing at all for a transparent PNG you can composite elsewhere.' },
    { key: 'art_symmetry', name: 'Symmetry fill', definition: 'A Chladni figure has the symmetry of its plate. With SYM on, filling one region also fills its rotated partners — 4 for a square, 6 for a hexagon, 2 for a rectangle — so a full colouring takes a few taps instead of dozens.' },
    { key: 'art_export', name: 'Export', definition: 'PNG and transparent PNG share or save the card; SVG writes the figure as editable vector paths; PDF prints it on Letter, A4 or a square page. ART PRINT is the figure alone. LAB PRINT adds the settings block so the sheet is a lab record — it always says Simulation. Controls without a native half are disabled until the next app build.' },
    { key: 'compare', name: 'Compare', definition: 'Two or four saved patterns on one canvas, each with its drive. Same frequency and a different figure means the object changed; same object and a different figure means the frequency did. That is the lab’s central discovery, made side by side.' },
    { key: 'lab_print', name: 'Lab print', definition: 'The settings block prints every value the figure came from — plate, thickness, edges, driver, support, damping, drive, resonance, level (or the dish and liquid, or the head and tension) — plus the date, your name for it, your notes and the Simulation label. A record, not a claim.' },
  ],
  commonMistakes: [
    ...
    'Expecting the liquid to pattern AT the drive frequency — Faraday waves answer at half of it; the RESP readout is the frequency you would measure on the surface.',
    'Pouring liquid into a loudspeaker to “try it at home” — the rig is a dish on a platform; liquid in a driver ruins it.',
  ],
  proTips: [
    ...
    'In the Liquid Studio, park SHAKE just above the threshold and swap liquids: the pattern vanishes for anything thicker until you shake harder — the threshold IS the viscosity lesson.',
