...
import { closeCenterLock, useTunerFrame } from '../../features/tools/tuner/tunerFrameStore';
import {
  buildTargets,
  centsBetween,
  CLOSE_CENTS,
...
export function CenterLockTuner() {
  ...
  const [a4, setA4] = useState(frame.a4 || 440);
  ...
  // ── targeting + lock (pure state machines stepped per pitch frame) ──
  ...
  useEffect(() => {
    // Reset the machines when the preset changes — the old target index means nothing now.
  ...
  useEffect(() => {
    ...
    unlockOrientation();
    return () => {
      void ka?.deactivateKeepAwake?.('centerlock');
      lockPortrait();
    };
  }, []);
  ...
  const bigSize = Math.round(Math.min(width, height) * (landscape ? 0.34 : 0.3));
  const meterW = Math.min(width - 32, 720);
  ...
  const pickString = (i: number) => {
    touch();
    if (manual && targetRef.current.target === i) {
      setManual(false);
    } else {
      targetRef.current = { target: i, candidate: null, candidateSince: null };
      lockRef.current = INITIAL_LOCK;
      setManual(true);
      setView((v) => ({ ...v, targetIdx: i, confirmed: false }));
    }
  };

  return (
    <Pressable style={styles.root} onPress={touch} accessible={false}>
      ...
      <View style={[styles.bar, { opacity: controlsShown ? 1 : 0.18 }]}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>
          ...
          <View style={styles.chipGap} />
          {A4_CHOICES.map((v) => (
            <Chip key={v} label={`A4 ${v}`} on={a4 === v} onPress={() => { touch(); setA4(v); }} small />
          ))}
          <View style={styles.chipGap} />
      ...
      <View style={[styles.main, landscape && styles.mainLandscape]}>
        <View style={styles.noteBlock} accessible accessibilityRole="text" accessibilityLabel={`${target.note}, ${direction}, ${fmtCents(cents)}`}>
          <Text style={[styles.identity, { color: tint }]}>
            {`${instrument === 'violin' ? '' : `STRING ${target.index} · `}${target.label}`}
          </Text>
          <View style={styles.noteRow}>
            ...
            <Text style={[styles.octave, { fontSize: Math.round(bigSize * 0.42), color: view.confirmed ? '#37e05f' : colors.textSecondary }]}>{octave}</Text>
          </View>
          <Text style={[styles.direction, { color: tint }]} accessibilityLiveRegion="polite">
            {direction}
          </Text>
        </View>

        <View style={styles.meterBlock}>
          {/* Converging meter: fixed −50…+50 ¢, flat left / sharp right, two
              wedges closing on the centre target, pointer at the damped value */}
      ...
      {/* String strip — per-string status; tap to lock (MANUAL), tap again for AUTO */}
      <View style={styles.strip}>
        {targets.map((t, i) => {
          ...
          return (
            <Pressable
              key={t.index}
              onPress={() => pickString(i)}
              style={[styles.stringKey, current && styles.stringKeyOn, current && { borderColor: tint }]}
      ...
      {/* Foot — input confidence, coaching, honesty */}
...
const styles = StyleSheet.create({
  root: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#050506', zIndex: 80, paddingTop: 44, paddingBottom: 18 },
  bar: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, gap: 8 },
  chips: { gap: 6, alignItems: 'center', paddingRight: 8 },
  chipGap: { width: 10 },
  ...
  main: { flex: 1, justifyContent: 'center', alignItems: 'center', gap: 14 },
  mainLandscape: { flexDirection: 'row', gap: 28, paddingHorizontal: 16 },
  ...
  octave: { fontFamily: fonts.oswaldMedium, marginLeft: 6, marginBottom: 14 },
  direction: { fontFamily: fonts.oswaldSemiBold, fontSize: 22, letterSpacing: 2.4, marginTop: 2 },
  ...
  strip: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', gap: 8, paddingHorizontal: 12, paddingVertical: 8 },
  stringKey: { minWidth: 58, paddingVertical: 8, paddingHorizontal: 10, borderRadius: 10, borderWidth: 1.5, borderColor: '#2a2b31', backgroundColor: '#101116', alignItems: 'center' },
  ...
  modeTag: { marginLeft: 6, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: '#2a2b31' },
