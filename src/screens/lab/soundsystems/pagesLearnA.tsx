 ...
 *
 * Every page with a live display and controls is a RACK page (owner
 * 2026-09-25): the instrument pinned on the glass, its readouts on the bezel,
 * its controls in the dock, the reading in the scroll well between them.
 * Pages that are reading or a set of checks keep the document layout. Copy
 * is the register a working engineer uses with a client: plain, technical,
 * no promises. "Live sound reinforcement" throughout.
 */
...
function PageSystem({ ctx }: { ctx: PageCtx }) {
  ...
  const params: DockParam[] = [
    flipFader({
      id: 'station',
      label: 'STATION',
      title: 'INSPECT A STATION',
      items: map.nodes,
      selectedId: sel ?? '',
      onSelect: tap,
      name: (n) => n.label,
      short: (n) => n.label.split(' ')[0],
      blurb: (n) => (n.kind === 'listener' ? 'The only station that cannot be replaced, moved or re-patched.' : gearSpec(n.kind).blurb),
      empty: 'ride to inspect a station',
    }),
    {
      kind: 'options',
      id: 'input',
      label: 'STAGE IN',
      valueLabel: v.input === 'snake' ? 'Snake' : 'Stagebox',
      options: [
        { id: 'snake', label: 'Analog snake', blurb: 'A multicore carries every microphone to the console at mic level, and the returns come back on the same loom. The preamps are at front of house.' },
        { id: 'stagebox', label: 'Digital stagebox', blurb: 'The preamps move to the stage; one network cable carries every channel to the console and the returns back — both ways on one link.' },
      ],
      selectedId: v.input,
      onSelect: (id) => set({ ...v, input: id as MapVariant['input'] }),
      sticky: true,
    },
    {
      kind: 'options',
      id: 'house',
      label: 'HOUSE',
      valueLabel: v.house === 'passive' ? 'Passive' : 'Powered',
      options: [
        { id: 'passive', label: 'Passive + amps', blurb: 'Main mix → processor → amplifiers → passive tops and subs, each amplifier channel on its own speaker cable.' },
        { id: 'powered', label: 'Powered boxes', blurb: 'The amplifier lives inside the cabinet: the main mix goes straight to the tops at line level, and the sub takes its own console output.' },
      ],
      selectedId: v.house,
      onSelect: (id) => set({ ...v, house: id as MapVariant['house'] }),
      sticky: true,
    },
  ];
  const inspect = spec ? (
  ...
  return (
    <SoundSystemsRackLayout
      rack={{
        ...
        hideDragTag: true,
        bezel: [
          { k: 'STAGE IN', v: v.input === 'snake' ? 'SNAKE' : 'STAGEBOX', flex: 1.2 },
          { k: 'HOUSE', v: v.house === 'passive' ? 'PASSIVE' : 'POWERED', flex: 1.2 },
          { k: 'STATION', v: node ? node.label.toUpperCase() : '—', tint: colors.cyanBright, flex: 1.5 },
          { k: 'SEEN', v: `${seen.size}`, flex: 0.7 },
        ],
        stage: (w, h) => (
          <StageFit w={w} h={h} aspect={MAP_W / MAP_H}>
            <SystemMap nodes={map.nodes} edges={map.edges} selectedId={sel} onTap={tap} a11y={a11y} />
          </StageFit>
        ),
        params,
      }}
      caption="Ride STATION along the signal path, or tap any station on the map. Switch the stage input and the house build and watch the map redraw — the order never changes."
      wellTop={inspect}
    >
...
function PageTrace({ ctx }: { ctx: PageCtx }) {
  ...
  const wrongText = wrongAt >= 0 ? `${STATION_LABEL[picked[wrongAt]]} cannot come after ${wrongAt === 0 ? 'the start' : STATION_LABEL[picked[wrongAt - 1]]} — the signal has not reached it yet. Reset and try again.` : null;
  const params: DockParam[] = [
    {
      kind: 'options',
      id: 'next',
      label: 'NEXT',
      valueLabel: last ? STATION_LABEL[last].split(' ')[0] : 'Pick',
      options: bin.map((id) => ({
        id,
        label: `${picked.includes(id) ? '✓ ' : ''}${STATION_LABEL[id]}`,
        blurb: id === last ? (wrongText ?? (complete ? 'Source to listener, in order.' : 'Lit. Now tap the next station the signal reaches.')) : picked.includes(id) ? 'Already lit.' : 'Tap to light it next.',
      })),
      selectedId: last,
      onSelect: (id) => {
        if (wrongAt >= 0 || picked.includes(id as Station) || complete) return;
        setPicked((p) => [...p, id as Station]);
      },
      sticky: true,
    },
    ...(picked.length ? [{ kind: 'action', id: 'reset', label: 'RESET', onPress: () => setPicked([]), tint: colors.red } as DockParam] : []),
  ];
  return (
    <SoundSystemsRackLayout
      rack={{
        ...
        initialParam: 'next',
        bezel: [
          { k: 'LIT', v: `${picked.length} / ${STATION_ORDER.length}` },
          { k: 'LAST', v: last ? STATION_LABEL[last].toUpperCase() : '—', flex: 1.6 },
          { k: 'THREAD', v: wrongAt >= 0 ? 'WRONG' : complete ? 'COMPLETE' : 'OK', tint: wrongAt >= 0 ? colors.red : complete ? colors.green : colors.amber },
        ],
        stage: (w, h) => (
          <StageFit w={w} h={h} aspect={MAP_W / MAP_H}>
      ...
      caption="Open NEXT and tap the stations in the order the signal travels — the map lights as you go, and stays open for the next one."
      wellTop={
        wrongAt >= 0 ? (
          <VerdictLine ok={false}>{wrongText}</VerdictLine>
        ) : complete ? (
    ...
    >
      ...
      <Body>The nine stations of the house path, drawn as the bench draws them. Light them in the order the signal travels.</Body>
...
function PageSystemTypes({ ctx }: { ctx: PageCtx }) {
  ...
  const params: DockParam[] = [
    flipFader({
      id: 'type',
      label: 'TYPE',
      title: 'TEN KINDS OF SYSTEM',
      items: SYSTEM_TYPES,
      selectedId: id,
      onSelect: pick,
      name: (s) => s.name,
      short: (s) => s.name.split(' ')[0],
      blurb: (s) => `${s.group === 'venue' ? 'CHOSEN BY THE VENUE' : 'A WAY TO BUILD ANY OF THEM'} · WHEN · ${s.when}`,
      sticky: true,
    }),
  ];
  return (
    <SoundSystemsRackLayout
      ...
      caption="Ride TYPE through the ten systems and read each plan — where the boxes stand and what the floor gets. Six are chosen by the venue; four are ways any of them can be built out."
    >
      <ChapterTag n={2}>PA SYSTEM TYPES</ChapterTag>
      <Body>{`${t.name} — plan view, stage at the top.`}</Body>
      <FieldKey />
      <GoalChips goals={goals} latched={latched} />
      <Card tone="math">
...
function PageConfigs({ ctx }: { ctx: PageCtx }) {
  ...
  const subs = bars.filter(([f]) => FEEDS[f].band === 'low');
  const params: DockParam[] = [
    flipFader({
      id: 'config',
      label: 'CONFIG',
      title: 'THIRTEEN OUTPUT CONFIGURATIONS',
      items: OUTPUT_CONFIGS,
      selectedId: id,
      onSelect: (o) => {
        setId(o);
  ...
  return (
    <SoundSystemsRackLayout
      ...
      caption="Ride CONFIG through the thirteen ways the console’s outputs can reach a room: the plan shows where each loudspeaker stands, the tag beside it says what it carries, and the floor shows where that lands."
    >
      <ChapterTag n={3}>OUTPUT CONFIGURATIONS</ChapterTag>
      <Body>The sectors are each loudspeaker’s nominal (−6 dB) coverage; the floor sums them in the conceptual model. Hatched floor is where two arrivals overlap and comb-filter.</Body>
      <FieldKey />
      <ReadoutRow>
        {bars.map(([feed, n]) => (
...
export const SS_LEARN_PAGES_A: SsPageDef[] = [
  { title: 'The complete system', short: 'SYSTEM', Component: PageSystem, manualDone: true, rack: true },
  { title: 'Trace the signal', short: 'TRACE', Component: PageTrace, manualDone: true, rack: true },
  { title: 'Ten kinds of system', short: 'TYPES', Component: PageSystemTypes, manualDone: true, rack: true },
  { title: 'Match the venue', short: 'MATCH', Component: PageVenueMatch, manualDone: true },
  { title: 'Output configurations', short: 'OUTPUTS', Component: PageConfigs, manualDone: true, rack: true },
