 ...
 * frame themselves. The DRAGGED display pins on the stage — Module 5 pins the
 * gain-staging scene (its one knob rides the dock lane; the tap-to-inspect
 * chain and S&H stations read in the well), Module 6 pins the float-headroom
 * bus (GAIN lane + TRIM key). Honesty badges stay with their displays
 * verbatim; the well carries its own guided-lesson entry row.
 *
 * NO Skia here: every animated view is reached ONLY through
  ...
  const params: DockParam[] = [
    {
      ...
      formatShort: () => `${driveDb >= 0 ? '+' : ''}${driveDb.toFixed(0)}dB`,
  ...
  return (
    <RackUnit
      ...
      stage={{
        ...
        bezel: [
          { k: 'PEAK', v: r.digitalOver ? '0.0 OVER' : `${r.recPeakDb.toFixed(1)} dBFS`, helpKey: 'gain_staging' },
          { k: 'SNR', v: `${r.snrDb.toFixed(0)} dB`, helpKey: 'gain_staging' },
          { k: 'HEADROOM', v: r.digitalOver ? 'none' : `${r.headroomDb.toFixed(1)} dB`, helpKey: 'gain_staging' },
          {
            k: 'ZONE',
            v: r.analogClip && r.digitalOver ? 'A+D CLIP' : r.analogClip ? 'AN. CLIP' : r.zone,
            tint: r.analogClip || r.digitalOver ? '#ff6b5e' : r.zone === 'RIGHT' ? '#5bff85' : undefined,
            helpKey: 'gain_staging',
          },
        ],
        render: (w, h) =>
          viz ? (
            <GainViz viz={viz} width={w} height={h} focused={focused} driveDb={driveDb} />
          ) : (
    ...
    >
      <View style={{ gap: 12 }}>
        {/* ── Gain staging (the star — its scene is pinned above) ── */}
...
function GainViz({
  ...
  return <viz.GainStagingView phase={phase} width={width} height={height} driveDb={driveDb} />;
}
  ...
  const params: DockParam[] = [
    {
      ...
      format: () => `${busDb >= 0 ? '+' : ''}${busDb.toFixed(1)} dBFS bus peak`,
      formatShort: () => `${busDb >= 0 ? '+' : ''}${busDb.toFixed(1)}`,
  ...
  return (
    <RackUnit
      ...
      stage={{
        ...
        bezel: [
          { k: 'FLOAT BUS', v: `${busDb >= 0 ? '+' : ''}${busDb.toFixed(1)}`, helpKey: 'gain_above_zero' },
          { k: 'FIXED', v: busDb > 0 ? '0.0 CLIP' : fixedDb.toFixed(1), tint: busDb > 0 ? '#ff6b5e' : undefined, helpKey: 'gain_above_zero' },
          { k: 'TRIM', v: trim ? `−${gainDb.toFixed(1)} dB` : 'OFF', helpKey: 'gain_above_zero' },
        ],
        render: (w, h) =>
          viz ? (
            <HeadroomViz viz={viz} width={w} height={h} focused={focused} gainDb={gainDb} trim={trim} />
          ) : (
            <View style={{ flex: 1, justifyContent: 'center', padding: 12 }}>
              <VizUnavailableCard />
            </View>
    ...
    >
      <View style={{ gap: 12 }}>
        {/* ── Gain above zero (the star — its bus is pinned above) ── */}
        <Text style={dstyles.body}>
          The same bus, two fates. Ride the GAIN lane, then hit TRIM and watch what each path
          gives back.
        </Text>
        <ReadoutGrid
          help={help}
...
function HeadroomViz({
  ...
  return <viz.FloatHeadroomView phase={phase} width={width} height={height} gainDb={gainDb} trim={trim} />;
}
