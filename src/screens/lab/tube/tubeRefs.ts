 ...
 *  `page` is 1 or 2; the underlying file is `<stem>-p<page>.png` unless an
 *  explicit `fileOverride` is given (e.g. a tube's `p1File` promo alias). */
export function tubePageUrl(stem: string, page: 1 | 2, fileOverride?: string): string {
  const file = fileOverride ?? `${stem}-p${page}.png`;
...
/** URL for a specific page of a tube, honoring its `p1File` override. */
export function tubeRefPageUrl(ref: TubeRef, page: 1 | 2): string {
  return tubePageUrl(ref.stem, page, page === 1 ? ref.p1File : undefined);
}
