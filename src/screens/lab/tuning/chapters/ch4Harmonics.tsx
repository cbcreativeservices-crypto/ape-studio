    ...
    <View style={{ gap: 12 }}>
      ...
      <Row>
        {(Object.keys(THIRDS) as Third[]).map((k) => (
          <Btn key={k} label={THIRDS[k].label} tone={!useSlider && third === k ? 'primary' : 'plain'} onPress={() => { setThird(k); setUseSlider(false); setSliderCents(THIRDS[k].value.cents); }} />
        ))}
      </Row>
      ...
      <Card tone="math">
        ...
        {!useSlider && third === 'just' ? (
          <>
            <MathLine>root ladder: f, 2f, 3f, 4f, 5f</MathLine>
            <MathLine>major third: (5/4)·f</MathLine>
          ...
          <>
            <MathLine>major third: 2^(1/3)·f = {ET_MAJOR_THIRD.decimalLabel}·f</MathLine>
          ...
          <>
            <MathLine>major third: (81/64)·f</MathLine>
          ...
          <>
            <MathLine>third ratio = 2^({cents.toFixed(2)}/1200) = {ratio.toFixed(6)}</MathLine>
      ...
      <Row>
        <Btn label="FULL TONES" tone={mode === 'full' ? 'primary' : 'plain'} onPress={() => setMode('full')} />
        <Btn label="ISOLATED PARTIALS" tone={mode === 'partials' ? 'primary' : 'plain'} onPress={() => setMode('partials')} />
      ...
      <Row>
        ...
        <Btn label="SHOW ME" onPress={() => { setUseSlider(true); setSliderCents(+JUST_MAJOR_THIRD.cents.toFixed(2)); }} />
      </Row>
      <SliderTrack value={sliderCents} onChange={(v) => { setUseSlider(true); setSliderCents(v); }} />
      <Text style={[styles.readout, { color: aligned ? colors.green : colors.textSecondary }]}>
      ...
      {aligned && useSlider ? <MarkOnce onMark={ctx.markDone} /> : null}

      <UnderstandingCheck
        question="Why do the compared harmonics align for a 5/4 major third?"
        options={['Because 5/4 is a small number', 'The fourth harmonic of the major third equals the fifth harmonic of the root', 'Because both notes share the same fundamental', 'Because 400 cents is a round number']}
        correct={1}
...
const styles = StyleSheet.create({
  ...
  trackLabel: { position: 'absolute', left: 6, bottom: 2, color: colors.textMutedDeep, fontFamily: fonts.barlowRegular, fontSize: 9 },
});
