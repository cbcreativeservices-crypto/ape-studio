 ...
 * interactive centerpiece (search for HARMONICS below).
 *
 ...
 *
 * History: this screen WAS the `EarLab` route (Phase-1 shell). On adopting the
 * v4 MASTER architecture (2026-07-26) the `EarLab` route became the Audio
 * Learning Lab landing menu and this harmonics experience moved to its own
 * `HarmonicLab` route — behavior unchanged.
 */
...
export function HarmonicLabScreen({ navigation }: Props) {
  const insets = useSafeAreaInsets();
  const { requestAudioOutput } = useAudioOutputGate();

  // Default to EXPLORE (spec) — the mode that hosts the harmonics view.
    ...
    <View style={[styles.root, { paddingTop: insets.top + 10 }]}>
      <View style={styles.header}>
        ...
        <View style={{ flexShrink: 1 }}>
          <Text style={styles.title}>HARMONIC LAB</Text>
          <Text style={styles.subtitle}>Additive Synthesis · Spectrum · Distortion</Text>
        </View>
      </View>
