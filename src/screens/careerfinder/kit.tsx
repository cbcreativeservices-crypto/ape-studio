...
export function SaveStar({ saved, onPress, name }: { saved: boolean; onPress: () => void; name: string }) {
  return (
    <Pressable onPress={onPress} style={styles.star} hitSlop={6} accessibilityRole="button" accessibilityState={{ selected: saved }} aria-pressed={saved} accessibilityLabel={saved ? `Remove ${name} from saved families` : `Save ${name}`}>
