  ...
  return (
    <FinderShell kicker="AUDIO CAREER FINDER · HOW THIS WORKS" title="What it measures, and what it doesn’t" onBack={() => navigation.goBack()} headerRight={<BetaPill />}>
      ...
      <Card>
        ...
        <Body>{fmt(CAREER_COUNT)} job titles were anchored to public occupational sources — the U.S. Bureau of Labor Statistics and O*NET, the Audio Engineering Society, the Acoustical Society of America, AVIXA, SMPTE, ASHA, IATSE, the Piano Technicians Guild and more than thirty others — and grouped into {FAMILY_COUNT} career families. Many titles are the same occupation under different names, seniority levels or employers; the index keeps them because that is how the industry actually talks.</Body>
        <Body>Each family names its three dominant activities. Your score on the primary activity carries {Math.round(FAMILY_WEIGHTS[0] * 100)}% of the family’s score, the secondary {Math.round(FAMILY_WEIGHTS[1] * 100)}% and the tertiary {Math.round(FAMILY_WEIGHTS[2] * 100)}%. Families are ranked from highest to lowest, and the reason each one appeared is generated from your actual answers by a fixed template — never by an AI.</Body>
        <Body>Profile clarity is a plain description of your answers: <Text style={styles.em}>Clear</Text> when at least {CLARITY_RULES.strongCount} activities scored high and the profile varies; <Text style={styles.em}>Broad</Text> when many were rated alike; <Text style={styles.em}>Developing</Text> when more than {Math.round(CLARITY_RULES.unknownShare * 100)}% of answers were “I don’t know”.</Body>
      </Card>
