...
// Guarded require: react-native-qrcode-svg is pure JS (renders via
// react-native-svg, which IS in the build), so it's safe — but we still guard it
// so a bad import can only fall back to the pending tile, never crash the app.
// (The launch crash was expo-iap's native call from the paywall, not this.)
