...
-- Any row that says FAIL or BLOCKED = stop and re-review.
--
...
select 'set' as section, k, v from (values
  ...
  ('users_total (pre-launch)',      (select count(*)::text from public.users))
) t(k,v);

-- ------------------------------------------------ 2. the split, computed live
-- CONVERT = the 17 explicit gs pairs below AND the names still match.
-- BLOCKED = active, no pair.   RETIRE = inactive, no pair.
...
v1 as (select * from public.achievements where course_id is not null),
j as (
  select v1.global_sequence v1_gs, v1.name v1_name, v1.is_active v1_active,
         v3.global_sequence v3_gs, v3.name v3_name, v3.is_active v3_active,
         (v3.id is not null and lower(btrim(v3.name)) = lower(btrim(v1.name))) name_ok
  from v1
...
select 'split' as section,
       case when v3_gs is not null then 'CONVERT' when v1_active then 'BLOCKED' else 'RETIRE' end as class,
       count(*) as topics
...
-- --------------------------------------------- 4. the 5 BLOCKED rows (no SQL)
-- These are ACTIVE, in use, and have NO v3 twin. This package deliberately does
-- NOT touch them. See NOTES_BLOCKED_FIVE.md. Nothing here is authored for them.
select 'blocked' as section, a.global_sequence gs, a.name,
...
-- ----------------------------------------------- 5. the full reference graph
-- Every table in the database that can hold an achievement id, and how many of
-- its rows point at one of the 51. This is derived from pg_constraint, not from
-- a hand-written list: the FK sweep below re-derives it at run time.
...
with v1 as (select id from public.achievements where course_id is not null),
pairs(v1_gs, v3_gs) as (values
  (0,3060),(2,3070),(4,3030),(8,3340),(12,3180),(14,3560),(15,3570),(16,3600),(22,3770),
  (32,3300),(35,3190),(37,4040),(41,4360),(46,4380),(47,4370),(49,4390),(50,4400)),
cls as (
  select a.id,
         case when p.v1_gs is not null then 'CONVERT'
              when a.is_active then 'BLOCKED' else 'RETIRE' end k
  from public.achievements a
  left join pairs p on p.v1_gs = a.global_sequence
...
select 'refs' as section, t.tbl,
       ...
       count(*) filter (where c.k='BLOCKED') blocked_rows,
       count(*) total
from (
...
-- NOTE: public_course_topics may already be gone. If the query above errors on
-- it, REMOVE_V1_REMNANTS Stage 60 has run — that is the expected end state, and
-- this package is happy either way. Re-run without that one UNION ALL line.

-- --------------------------------------------------- 6. collisions on convert
...
select 'collisions' as section, k, v from (values
  ...
  ('glossary_topics · v1 has difficulty the v3 twin lacks (expect 0 - no data lost on delete)',
     (select count(*) from public.glossary_topics s join m on m.v1id=s.achievement_id
...
-- ------------------------------------ 7. the two content sets NOT being moved
...
-- ----------------------------------------------------- 8. sequencing blockers
select 'sequencing' as section, item, state from (values
  ('REMOVE_COLLEGE_COURSES_2026_09_03 (row delete in public_course_topics) - optional here',
...
-- --------------------------------------------- 9. trigger / constraint hazards
select 'hazards' as section, k, v from (values
  ('achievements BEFORE UPDATE trig_validate_applicable_methods would reject a bad method key',
  ...
  ('achievements BEFORE UPDATE trig_validate_quiz_question_count only fires on ACTIVATION',
     'Stage 30 only ever sets is_active=false, so it cannot fire. Safe.'),
  ('student_achievement_progress AFTER trigger student_progress_award fires on INSERT or UPDATE OF status',
     'Stage 10 updates achievement_id only, so it does NOT fire. Stage 10 therefore calls evaluate_user_credentials() explicitly for every affected user.'),
  ('glossary_topics FK is ON DELETE CASCADE',
     (select count(*)::text || ' glossary_topics rows would be silently destroyed by a hard DELETE of the 51. This is the main reason Stage 30 is a soft retire.'
