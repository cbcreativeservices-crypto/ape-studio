...
--
-- Stage 30 already satisfies the ruling. This stage exists only because you may
-- prefer the 46 rows physically removed. It will REFUSE to run while anything
-- still points at them, and today it WILL refuse, because:
--
...
--   * 3 badges (MIC Certified, REC Certified, MIX Certified) trigger off RETIRE
--     topics. Same NO ACTION FK, same decision: delete the badge or retarget it.
...
--
-- The 5 BLOCKED topics are never deleted by this stage.
...
begin
  ...
  if to_regclass('public.cr_v1topics_ledger_20260903') is null
     or (select count(*) from public.cr_v1topics_ledger_20260903) <> 46 then
    raise exception 'STAGE 40 ABORTED: Stage 30 has not run - retire before you delete';
  end if;

  -- ------------------------------------------------ every inbound reference
  select count(*) into v_n from public.glossary_topics x
    join public.cr_v1topics_map_20260903 m on m.v1_id=x.achievement_id where m.class in ('CONVERT','RETIRE');
  ...
  select count(*) into v_n from public.badges x
    join public.cr_v1topics_map_20260903 m on m.v1_id=x.trigger_achievement_id where m.class in ('CONVERT','RETIRE');
  ...
  if to_regclass('public.public_course_topics') is not null then
    execute 'select count(*) from public.public_course_topics x
             ...
             where m.class in (''CONVERT'',''RETIRE'')' into v_n;
    if v_n > 0 then
  ...
  if exists (
    ...
    raise exception 'STAGE 40 ABORTED: a foreign key into achievements exists that this file does not check. Re-run 00_PRECHECK section 5 and update the guards.';
  end if;

  delete from public.achievements a
   using public.cr_v1topics_map_20260903 m
   where m.v1_id = a.id and m.class in ('CONVERT','RETIRE');
  get diagnostics v_del = row_count;

  insert into public.cr_v1topics_report_20260903 (stage, k, v) values
    ('40_APPLY','achievements_hard_deleted', v_del::text),
    ('40_APPLY','v1_rows_remaining',
